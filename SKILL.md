...
**当此 skill 被触发时，必须先执行以下流程：**

1. **欢迎语**：向用户介绍本工具的核心功能
2. **功能菜单**：使用 AskUserQuestion 工具展示以下选项让用户选择：

   - **查询价格**：查看所有模型价格或按关键词筛选（claude、gpt、gemini 等）
   - **查询余额与用量**：查看账户余额 + 用量 Top3 + 价格 + 替代建议（首次需登录一次）
   - **性价比推荐**：获取整体性价比最高的模型列表
   - **价格对比说明**：了解 Aiberm 两套价格体系的区别

3. **执行操作**：根据用户选择，调用对应的 Bash 命令执行查询
4. **后续引导**：操作完成后，询问用户是否需要其他查询

**示例对话流程**：

```
Assistant: 欢迎使用 Aiberm 价格监控工具！

这个工具可以帮你：
• 实时查询 Aiberm 所有模型的最新价格
• 查看账户余额和用量统计
• 推荐性价比最高的模型
• 为常用模型找到更便宜的替代品
